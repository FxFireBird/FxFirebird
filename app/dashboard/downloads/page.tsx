...
const topCards = [
  { label: "Subscription Plan", value: "Full Suite", icon: CreditCard, accent: "from-orange-500 to-orange-600" },
  { label: "Active Markets", value: "Forex & Synthetics", icon: Activity, accent: "from-green-500 to-emerald-600" },
  { label: "License Status", value: "Active", icon: Shield, accent: "from-blue-500 to-cyan-600" },
  { label: "Next Billing", value: "Jan 15, 2025", icon: Calendar, accent: "from-purple-500 to-violet-600" },
]
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {topCards.map((card) => (
          <motion.div key={card.label} variants={item} className="bg-card/50 backdrop-blur-sm border border-border rounded-2xl p-5 hover:border-orange-500/30 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-card/50 backdrop-blur-sm border border-border rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
          ...
          <div className="mt-4 space-y-2">
            {quickActions.map((action) => (
              <a key={action.label} href={action.href} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-orange-500/10 text-muted-foreground hover:text-orange-500 transition-all group">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-2xl p-6 flex items-center justify-between flex-wrap gap-4">
        ...
        <a href="https://status.fxfirebird.com" target="_blank" rel="noopener noreferrer" className="text-sm text-green-500 hover:text-green-400 transition-colors">
